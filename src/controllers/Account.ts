...
import { default as Account, AccountModel, AuthToken } from '../models/Account';
...
export let postLogin = (req: Request, res: Response, next: NextFunction) => {
  ...
  passport.authenticate(
    'local',
    (err: Error, user: AccountModel, info: IVerifyOptions) => {
      if (err) {
        return next(err);
      }
      if (!user) {
...
export let postUpdateProfile = (
  ...
  Account.findById(req.user.id, (err, user: AccountModel) => {
    if (err) {
      return next(err);
    }
...
export let postDeleteAccount = (
  ...
  Account.remove({ _id: req.user.id }, err => {
    if (err) {
      return next(err);
    }
    req.logout();
...
export let getOauthUnlink = (
  ...
  Account.findById(req.user.id, (err, user: any) => {
    if (err) {
      return next(err);
    }
...
export let postReset = (req: Request, res: Response, next: NextFunction) => {
  ...
  async.waterfall(
    [
      ...
      function sendResetPasswordEmail(user: AccountModel, done: Function) {
  ...
  async.waterfall(
    [
      ...
      function setRandomToken(token: AuthToken, done: Function) {
        Account.findOne({ email: req.body.email }, (err, user: any) => {
          if (err) {
            return done(err);
          }
          if (!user) {
